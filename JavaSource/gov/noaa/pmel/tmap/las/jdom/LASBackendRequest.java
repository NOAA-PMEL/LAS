...
/**
 * An instantiation of the LAS Backend Request XML.  For example:
 * &lt;backend_request&gt; etc. etc.
...
public class LASBackendRequest extends LASDocument {
        ...
        for (Iterator doIt = dataObjects.iterator(); doIt.hasNext();) {
            ...
            for (Iterator dataIt = dataList.iterator(); dataIt.hasNext();) {
                ...
                for (Iterator attIt = attributes.iterator(); attIt.hasNext();) {
                    ...
                    // See if this variable has an assoicated transformation...
    ...
    /* 
     * Get all the "global" properties as sybmol strings.
     */
    ...
    /* Get the "global" properties for a particular property group
     * as symbol Strings.
     */
    public HashMap<String, String> getSymbols (Element group, int index) {
     ...
     /* geta all of the property groups from a particular data object
     * as a list of symbol strings.
     */
     ...
     * @param lasBackendRequest - the original request to be modified.
     * @param output_dir - the directory where the local results should be written.
     * @param base_url - the base URL of the remote server which is running the service and will serve the remote files.
     */
    public void setLocalFileNames(String output_dir, String http_base_url, String opendap_base_url) throws JDOMException {
        ...
        for (Iterator resIt = results.iterator(); resIt.hasNext();) {
            Element result = (Element) resIt.next();
                ...
                // Java on unix cannot.  So always use "/" and everybody's happy.
                if ( file.contains("\\") ) {
