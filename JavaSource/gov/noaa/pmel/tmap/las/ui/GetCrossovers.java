...
public class GetCrossovers extends LASAction {
    ...
    public static final double MAX_FCO2_DIFF = 5.0d;
    public static final double MAX_TEMP_DIFF = 0.3d;
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm form,
	    ...
	    List<String> candidateCruises = new ArrayList<String>();
	                        ...
	                        if ( id != null && !id.equals("") ) {
	                            ...
	                            if ( table != null && !table.equals("") ) {
	                                ...
	                                try {
	                                    ...
	                                    // Call out to ERDDAP for all the valid CRUISES in the same lat/time box, 
	                                    // but you have to fudge the time and lat because they only have to be "close"
	                                    ...
	                                    // Valid cruise means qc_flag not S (suspended) or X (excluded) and different ship ID
	                                    String crossoversurl = dataurl + ".json?"+URLEncoder.encode(traj_id_name+"&"+traj_id_name+"!=\""+tid+"\"&distinct()&time>="+tmin+"&time<="+tmax+
	                                            "&latitude>="+latitudeMin+"&latitude<="+latitudeMax+"&qc_flag!=\"X\"&qc_flag!=\"S\"", "UTF-8");
	                                    stream = null;
	                                    ...
	                                    if ( stream != null ) {
	                                        ...
	                                        // Ship identified by the first four characters of the trajectory ID
	                                        ...
	                                        for (int i = 0; i < rows.size(); i++) {
	                                            ...
	                                            if ( !candidate_id.startsWith(filter)) {
	                                                candidateCruises.add(candidate_id);
	                                            }
	                                        }
	                                        stream.close();
	                                    } else {
	                                ...
	                                if ( candidateCruises.size() > 0 ) {
	                                    ...
	                                    if ( selectedCruise != null ) {
	                                        for (Iterator<String> cIt = candidateCruises.iterator(); cIt.hasNext();) {
	                                            String cid = cIt.next();
    ...
    public Crossover checkCrossover(JsonObject selectedCruise,  JsonObject potentialCross) {
	    ...
	    for (int i = 0; i < rows.size(); i++) {
            ...
            for ( int j = 0; j < crossingRows.size(); j++ ) {
                ...
                if ( Math.abs(crossingTemp - temp) > MAX_TEMP_DIFF ) {
                ...
                if ( Math.abs(crossingfCO2 - fCO2) > MAX_FCO2_DIFF ) {
                ...
                if ( (locTimeDist < minDistance) && (locTimeDist <= CUTOFF) ) {
