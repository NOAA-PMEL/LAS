...
public class UI extends BaseUI {
    ...
    WidgetSelectionChangeEvent.Handler updateNeededEventHandler = new WidgetSelectionChangeEvent.Handler() {
        ...
        public void onAxisSelectionChange(WidgetSelectionChangeEvent event) {
            ...
            if ( applyButton.getCheckBoxValue() || event.isAuto() || required_update ) {
    ...
    public void onModuleLoad() {
        ...
        annotationsControl = new ToggleButton("Plot Info", annotationsClickHandler);
        ...
        eventBus.addHandler(VariablePluralityEvent.TYPE, new VariablePluralityEvent.Handler() {
            ...
            public void onPluralityChange(VariablePluralityEvent event) {
                ...
                if ( event.isPlural() ) {
                    // Disable compareMenu and provide tool tip to explain why
                    compareMenu.setEnabled(false);
                    compareMenu.setTitle("Comparisons are only available for plots and charts of one variable. Remove such variables on the offending panel(s) to re-enable this control.");
    ...
    private void popHistory(String historyToken) {
        if ( !historyToken.equals("") ) {
            ...
            if ( (tokenMap.containsKey("xDSID") && tokenMap.get("xDSID").equals(xVariable.getDSID())) && (tokenMap.containsKey("varid") && tokenMap.get("varid").equals(xVariable.getID())) ) {
