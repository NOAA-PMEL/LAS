...
public class UI extends BaseUI {
	...
	// Everybody that sub-classes BaseUI must implement three handlers for
	// options OK, operations clicks and data set selections.
	...
	public void onModuleLoad() {
		...
		compareMenu.addChangeHandler(new ChangeHandler() {
			...
			public void onChange(ChangeEvent event) {
				...
				if (p.equals("1")) {
					initPanels(true);
					Widget compareButtons = xButtonLayout.getWidget(0,
							getButtonIndex());
					xButtonLayout.remove(compareButtons);
					xButtonLayout.setWidget(0, getButtonIndex(),
							tOperationsMenu);
				} else {
					initPanels();
		...
		annotationsControl = new ToggleButton(new Image(GWT.getModuleBaseURL()
				+ "../images/i_off.png"), new Image(GWT.getModuleBaseURL()
				+ "../images/i_on.png"), annotationsClickHandler);
		...
		// variable needed.
		if (xDSID != null && xVarID != null & xOperationID != null
				&& xView != null) {
			// If the proper information was sent to the widget, pull down the
			// variable definition
	...
	private void turnOffAnalysis() {
		...
		xOperationID = ops[0].getID();
		xOperationsWidget.setOperations(xVariable.getIntervals(),
		...
		public void onSuccess(Object result) {
			...
			if (cats.length > 1) {
				...
				if (cats[0].isVariableChildren()) {
					...
					Util.getRPCService().getGrid(xDSID, xVarID,
							initVizGalForHistory);
				} else {
		...
		if (initialHistory != null && !initialHistory.equals("")) {
			...
			for (int t = 0; t < xPanels.size(); t++) {
				...
				if (t == 0) {
					...
					xOperationsWidget.setOperation(
							tokenMap.get("operation_id"),
							panelTokenMap.get("xView"));
					xAxesWidget.getRefMap().setCurrentSelection(
							Double.valueOf(panelTokenMap.get("ylo")),
	...
	public boolean init() {
		...
		xOperationsWidget.setOperations(xVariable.getIntervals(), xOperationID,
				xView, ops);
		tOperationsMenu.setMenus(ops, xView);
		xOptionsButton.setOptions(xOperationsWidget.getCurrentOperation()
				.getOptionsID());
		...
		if (differenceButton.isDown()) {
			...
			for (Iterator panelIt = xPanels.iterator(); panelIt.hasNext();) {
				...
				if (!panel.getID().equals(comparePanel.getID())) {
					...
					panel.setAnalysis(a);
					panel.computeDifference(xOptionsButton.getState(),
							switchAxis, force);
				}
			}
	...
	// panel.setAxisRangeValues("z", xAxesWidget.getZAxis().getLo(),
	// xAxesWidget.getZAxis().getHi());
	// }
	// } else if ( xView.equals("t") ) {
	// if ( xVariable.getGrid().hasT() ) {
	// panel.setAxisRangeValues("t", xAxesWidget.getTAxis().getFerretDateLo(),
	// xAxesWidget.getTAxis().getFerretDateHi());
	// }
	...
	public void changeDataset() {
		...
		// TODO Maybe we can derive the default operations from the data set
		// during the init(), but it would require an asynchronous request
		...
		} else if (xNewVariable.getAttributes().get("grid_type")
				.equals("vector")) {
	...
	public void setFromHistoryToken(Map<String, String> tokenMap,
		...
		xOperationsWidget.setOperation(tokenMap.get("operation_id"),
				tokenMap.get("view"));
		// s, n, w, e
		xAxesWidget.getRefMap().setCurrentSelection(
	...
	private void popHistory(String historyToken) {
		if (historyToken.equals("")) {
			...
			/*
			 * 
			 * Things that have to be reconciled: was the panel in "panel mode"
			 * if yes: reinitialize the panel to the ds and var in the history
			 * token then set its state if no: is the ds and var the same as the
			 * current ds and var if yes: set the state if no: reinitialize with
			 * ds and var, then set the state
			 */

			// Check to see if the the variable is the same as before and that
			// no panels are in usePanel mode.
			// This variable will be true if the settings only apply to the
			// gallery state and not the variable
			...
			if ((tokenMap.containsKey("xDSID") && tokenMap.get("xDSID").equals(
				...
				/*
				 * Apply the settings to the panels (the compare axis and some
				 * plot options). The first token string (index 0) is all the
				 * junk in the header so we start counting the tokens with t+1.
				 * 
				 * The rest are the tokens for the individual panels. We must
				 * apply the first (index 0) to the gallery ControlSettings
				 * widget then the panel.
				 */
				...
				for (int t = 0; t < xPanels.size(); t++) {
					...
					xPanels.get(t).setFromHistoryToken(panelTokenMap,
							optionsMap);
				}
			} else {
				// If the variable happens to be the initial variable use it.
				if ((tokenMap.containsKey("xDSID") && tokenMap.get("xDSID")
						.equals(initial_var.getDSID()))
						&& (tokenMap.containsKey("varid") && tokenMap.get(
								"varid").equals(initial_var.getID()))) {
					initPanels();
					switch_axis = applyHistory(tokenMap);
				} else {
					// Otherwise, save the history tokens, call back to the
					// server for the variable and then apply the history in the
					// callback.
					historyTokens = tokenMap;
					Util.getRPCService().getGrid(tokenMap.get("xDSID"),
							tokenMap.get("vid"), requestGridForHistory);
				}
			}
	...
	private void applyTokensToPanelModePanels(String[] settings) {
		/*
		 * There are 5 sets of tokens. 0. Gallery 1. Panel 0 (upper left) never
		 * in "panel" mode. 2. Panel 1 (upper right) 3. Panel 2 (lower left) 4.
		 * Panel 3 (lower right)
		 */

		for (int t = 2; t < settings.length; t++) {
			HashMap<String, String> panelTokenMap = Util
					.getTokenMap(settings[t]);
			HashMap<String, String> optionsMap = Util
					.getOptionsMap(settings[t]);
			OutputPanel panel = xPanels.get(t - 1);
			// If this history token has a panel that is using it's own
			// settings, update witht the token.
			if (panelTokenMap.get("usePanelSettings").equals("true")) {
				String did = panelTokenMap.get("dsid");
				String vid = panelTokenMap.get("varid");
				boolean change = ((!did.equals(xVariable.getDSID())) || (!vid
						.equals(xVariable.getID())));
				panel.setPanelModeFromHistoryToken(panelTokenMap, optionsMap,
						change);
			}
		}
	}
	...
	private boolean applyHistory(Map<String, String> tokenMap) {
		...
		if (tokenMap.containsKey("fixedAxis")
				&& tokenMap.containsKey("fixedAxisLo")
				&& tokenMap.containsKey("fixedAxisHi")) {
			// String new_fixedAxis = tokenMap.get("fixedAxis");
			// if ( !new_fixedAxis.equals(fixedAxis) ) {
			// switch_axis = true;
			// if ( tokenMap.containsKey("compareAxis") &&
			// tokenMap.get("compareAxis") != null ) {
