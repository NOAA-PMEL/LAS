...
public class VariableSelectorImpl extends VerticalPanel implements VariableSelector {
    ...
    public void addListBox(UserListBox newListBox) {
        ...
        if ( itemCount == 0 ) {
            ...
            for ( int b = 0; b < itemCount; b++ ) {
                ...
                String svarid = box.getUserObject(box.getSelectedIndex()).getID();
                box.removeVectors();
                
                int si = 0;
                for ( int c = 0; c < box.getVariables().size(); c++ ) {
                    String nid = box.getVariables().get(c).getID();
                    if ( nid.equals(svarid)) si = c;
                }
    ...
    public void removeListBox(UserListBox oldListBox) {
        ...
        if ( latestListBox != null ) {
            ...
            Vector<VariableSerializable> variables = latestListBox.getVariables();
            if ( variables != null ) {
    ...
    /**
     * Need to be able to remove the list boxes without firing any events except to fire one at the end.
     */
    @Override
    ...
    public void setVariables(Vector<VariableSerializable> variables, int selected) {
        ...
        if ( !isComparing() && isOnComparePanel() && (variables != null) && (variables.size() > 1) ) {
    ...
    @Override
    public void setVariableMetadataView(VariableMetadataView variableMetadataView) {
