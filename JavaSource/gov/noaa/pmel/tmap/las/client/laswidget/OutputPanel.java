...
public class OutputPanel extends Composite implements HasName {
    ...
    private RequestCallback lasRequestCallback = new RequestCallback() {
        ...
        public void onResponseReceived(Request request, Response response) {
            logger.info("enterring lasRequestCallback.onResponseReceived request:" + request + "\nresponse:" + response);
                ...
                for ( int n = 0; n < results.getLength(); n++ ) {
                    if ( results.item(n) instanceof Element ) {
                        ...
                        if ( typeAttribute.equals("image") ) {
                            ...
                            if ( currentURL.contains("cancel") ) url = url+"&cancel=true";
    ...
    public MapSelectionChangeListener mapListener = new MapSelectionChangeListener() {
        ...
        public void onFeatureChanged() {
            ...
            eventBus.fireEventFromSource(new MapChangeEvent(panelAxesWidgets.getRefMap().getYlo(), panelAxesWidgets.getRefMap().getYhi(), panelAxesWidgets.getRefMap().getXlo(), panelAxesWidgets.getRefMap().getXhi()),
                    thisOutputPanel);
    ...
    public void computeDifference(Map<String, String> options, boolean switchAxis, boolean force) {
     ...
     // If the passed in variable is a vector, then the panel variable must
        ...
        if ( vizGalVariable.isVector() ) {
            ...
            lasRequest.addVariable(var.getDSID(), var.getComponents().get(0), 1);
            if ( view.contains("x") ) {
    ...
    public String getHistoryToken() {
        ...
        if ( analysis == null ) {
            ...
            wantZ = var.getGrid().hasZ() && !view.contains("z") && !analysis.isActive("z");
        }
        if ( wantZ ) {
        ...
        if ( analysis == null ) {
            ...
            wantT = var.getGrid().hasT() && !view.contains("t") && !analysis.isActive("t");
        }
        if ( wantT ) {
    ...
    public void refreshPlot(Map<String, String> options, boolean switchAxis, boolean popup, boolean force) {
        ...
        if ( var.getGrid().hasT() ) {
            if ( view.contains("t") && lasRequest.getRangeLo("t", 0).equals(lasRequest.getRangeHi("t", 0)) ) {
                ...
                messagePanel.setWidth(w + "px");
                messagePanel.show(plotWidget.getAbsoluteLeft() + 15, plotWidget.getAbsoluteTop() + 15, "Set the range selectors for the Date/Time to different values and click the update plots button.");
                return;
            }
        ...
        if ( var.getGrid().hasZ() ) {
            if ( view.contains("z") && lasRequest.getRangeLo("z", 0).equals(lasRequest.getRangeHi("z", 0)) ) {
                ...
                messagePanel.setWidth(w + "px");
                messagePanel.show(plotWidget.getAbsoluteLeft() + 15, plotWidget.getAbsoluteTop() + 15, "Set the range selectors for the Z-Level to different values and click the Apply button.");
                return;
            }
        }
    ...
    private void setOrthogonalAxesValues() {
        ...
        if ( !view.contains("x") && !view.contains("y") ) {
            ...
            setLatLon(String.valueOf(panelAxesWidgets.getRefMap().getYlo()), String.valueOf(panelAxesWidgets.getRefMap().getYhi()), historyTokenMap.get("xlo"), historyTokenMap.get("xhi"));
        } else if ( view.contains("x") && !view.contains("y") ) {
            setLatLon(historyTokenMap.get("ylo"), historyTokenMap.get("yhi"), String.valueOf(panelAxesWidgets.getRefMap().getXlo()), String.valueOf(panelAxesWidgets.getRefMap().getXhi()));
        }
    }
    ...
    public void setVariable(VariableSerializable v) {
        ...
        UserListBox box = getOutputControlPanel().getVariableControls().getMultiVariableSelector().getVariableSelector().getLatestListBox();
        int index = 0;
        for ( int vi = 0; vi < box.getVariables().size(); vi++) {
            VariableSerializable vs = (VariableSerializable) box.getVariables().get(vi);
