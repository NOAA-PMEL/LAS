...
function moveHorizonalLine (evt) {
   ...
   if (document.all) {
     ...
     if (X1 < X0)
       r.style.pixelLeft = X1;
     else
       r.style.pixelLeft = X0;

     r.style.pixelTop = Y0;

     r.style.width = Math.abs(X1 - X0);
     r.style.height = 0;
     r.style.minHeight = 0;
   }
   else if (document.getElementById) {
...
function displayBox() {
   ...
   //for test
//   window.status = "in displayBox(): " + r.style;
...
function startDrawing (evt) {
   ...
   //for test
//   window.status = "in startDrawing(): " + currView;
